...
init([]) ->
    ...
    send_message("connect_publisher", "sentiment", ""),
    send_message("connect_publisher", "engagement", ""),
    send_message("connect_publisher", "tweet", ""),
    {ok, Socket}.

send_message(Command, Topic, Msg) ->
    EncodedJson = transform_to_json(Command, Topic, Msg),
    io:format("~p ~n", [jsx:decode(EncodedJson)]),
